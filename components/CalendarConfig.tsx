...
const CalendarConfig: React.FC<CalendarConfigProps> = ({ lang, onClose }) => {
    const [calendars, setCalendars] = useState<{ id: string; summary: string; primary?: boolean; backgroundColor?: string; foregroundColor?: string }[]>([]);
    ...
    const loadData = async () => {
        ...
        try {
            ...
            setSelectedIds(new Set(prefs.calendars.map(c => c.id)));
        } catch (error) {
    ...
    const handleSave = async () => {
        setIsSaving(true);
        try {
            // Filter available calendars to get full objects for selected IDs
